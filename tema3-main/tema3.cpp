...
void type0_computations(int rank, int num_procs, int assigned_coordinator, int vec_elems, int** topology) {
    ...
    if (rank == 0) {
        ...
        for (int i = 1; i <= topology[rank][0]; i++) {
            int curr_worker = topology[rank][i];
            send_and_log(&tasks_per_worker, 1, MPI_INT, i, rank, WORKER_TAG, MPI_COMM_WORLD);
            send_and_log(&vec[offset], tasks_per_worker, MPI_INT, i, rank, WORKER_TAG, MPI_COMM_WORLD);

            offset += tasks_per_worker;
        }

        // send tasks to coordinators (first inform about length and offset, then send vec)
        for (int i = 1; i < num_coord; i++) {
            start_indexes[i] = offset;
            int num_operations = tasks_per_worker * topology[i][0];
            send_and_log(&num_operations, 1, MPI_INT, i, rank, RING_TAG, MPI_COMM_WORLD);
            send_and_log(&vec[offset], num_operations, MPI_INT, i, rank, RING_TAG, MPI_COMM_WORLD);
        ...
        for (int i = 0; i < topology[rank][0]; i++) {
            ...
            offset = find_worker_offset(rank, status.MPI_SOURCE, topology);
        ...
        for (int i = 1; i < num_coord; i++) {
            int num_operations = tasks_per_worker * topology[i][0];
            MPI_Status status;
            MPI_Recv(recv_vec, num_operations, MPI_INT, i, RING_TAG, MPI_COMM_WORLD, &status);
    ...
    if (rank > 0 && rank < num_coord) {
        int num_operations;
        MPI_Status status;
        MPI_Recv(&num_operations, 1, MPI_INT, 0, RING_TAG, MPI_COMM_WORLD, &status);

        int tasks_per_worker = num_operations / topology[rank][0], offset = 0;
        int* recv_vec = (int*) calloc(sizeof(int), num_operations);
        int* worker_vec = (int*) calloc(sizeof(int), tasks_per_worker);
        MPI_Recv(recv_vec, num_operations, MPI_INT, 0, RING_TAG, MPI_COMM_WORLD, &status);

        // assign tasks to workers
        for (int i = 1; i <= topology[rank][0]; i++) {
            int curr_worker = topology[rank][i];
            send_and_log(&tasks_per_worker, 1, MPI_INT, i, rank, WORKER_TAG, MPI_COMM_WORLD);
            send_and_log(&recv_vec[offset], tasks_per_worker, MPI_INT, i, rank, WORKER_TAG, MPI_COMM_WORLD);
        ...
        for (int i = 0; i < topology[rank][0]; i++) {
            ...
            offset = find_worker_offset(rank, status.MPI_SOURCE, topology);
            for (int j = 0; j < tasks_per_worker; j++) {
                recv_vec[j + offset] = worker_vec[j];
                worker_vec[j] = 0;
            }
        }

        // send final vec to leader
        send_and_log(recv_vec, num_operations, MPI_INT, 0, rank, RING_TAG, MPI_COMM_WORLD);
    }
